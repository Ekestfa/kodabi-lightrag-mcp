## Requirement: `Provide Basic Access to LightRAG via HTTP Server`
- **ID**: REQ202511272348
- **Name**: `Provide Basic Access to LightRAG via HTTP Server`
...
### Use Cases
#### Use Case 1: Query Different RAG Microservices with Specified Port Numbers (REQ202511272348-US01)
- **Primary Actor**: LightRAG User
- **Stakeholders**: Central RAG Server, RAG Microservices (e.g., Software Engineering RAG, History RAG)
- **Preconditions**:
  - A central RAG server is configured with access to multiple RAG microservices.
  - Each microservice is running on a distinct port (e.g., Software Engineering RAG on port 8080, History RAG on port 8081).
  - The user has a valid API token and base URL configured for authentication.
- **Postconditions**:
  - The central RAG server successfully routes the query to the correct microservice.
  - A contextual response with source document information is returned to the user.
- **Main Success Scenario**:
  1. The user sends a query to the central RAG server specifying a microservice name and its associated port number.
  2. The server validates the microservice name and port number.
  3. The server checks if the microservice exists in its configuration.
  4. If valid, the server routes the query to the microservice on the specified port.
  5. The microservice retrieves context from its vector database.
  6. The microservice returns a contextual response with source document information.
  7. The central server assembles the response with metadata and sends it to the user.
- **Alternative Flows**:
  - **Invalid Port**: If the port number is invalid or non-existent, the server returns a clear error message indicating the invalid port and lists all available microservices with their port numbers.
  - **Missing Microservice**: If the requested microservice does not exist in the configured list, the server returns a message stating the microservice is not found and lists all available microservices.
  - **Malformed Input**: If the query is malformed or missing a required parameter (e.g., port), the server returns an error message indicating the port number is required and does not attempt to connect to any microservice.
- **Business Rules and Constraints**:
  - All port numbers must be integers within a valid range (e.g., 8000–9999).
  - The microservice name must match one of the predefined service names in the configuration.
  - The central server must validate both the microservice name and port number before routing.
- **Technical Dependencies**:
  - The central RAG server must use a routing mechanism to direct queries to the correct microservice port.
  - The microservices must be accessible via HTTP on their designated ports.
  - The RAG system must support authentication via API token and base URL.
#### Use Case 2: Health Check via `/health` API (REQ202511272348-US02)
- **Primary Actor**: LightRAG User
- **Stakeholders**: Central RAG Server, Workflow Engine
- **Preconditions**:
  - The web server is running and the `/health` endpoint is active.
  - The user has initiated a RAG workflow.
- **Postconditions**:
  - The workflow proceeds with RAG operations if the server is healthy.
  - The workflow is halted if the server is unavailable.
- **Main Success Scenario**:
  1. The user initiates a RAG workflow.
  2. The system sends an HTTP GET request to the `/health` endpoint.
  3. The server returns a `200 OK` response indicating it is up and healthy.
  4. The workflow proceeds with RAG operations.
- **Alternative Flows**:
  - **Server Unavailable**: If the server returns a `503 Service Unavailable` or `404 Not Found` response, the workflow is halted with a clear error message indicating the server is not available.
  - **Malformed Response**: If the `/health` endpoint returns a non-JSON or malformed response, the system logs a warning and triggers a fallback mechanism to check alternative health endpoints or restart the server process.
  - **Network Timeout**: If the server has a network timeout when attempting to reach the `/health` endpoint, the system responds with a `504 Gateway Timeout` error and logs the failure, preventing the workflow from starting until the issue is resolved.
- **Business Rules and Constraints**:
  - The `/health` endpoint must return a `200 OK` response when the server is operational.
  - The system must not proceed with RAG operations if the server is not confirmed to be healthy.
- **Technical Dependencies**:
  - The `/health` endpoint must be accessible via HTTP GET at a defined path.
  - The server must have a reliable mechanism to detect its own operational status.
#### Use Case 3: Query RAG via Web Server (`/query` API) (REQ202511272348-US03)
- **Primary Actor**: LightRAG User
- **Stakeholders**: Web Server, RAG System, Vector Database
- **Preconditions**:
  - The LightRAG system is configured with a valid RAG service endpoint and API token.
  - The web server is running at `http://[::1]:8080`.
- **Postconditions**:
  - The user receives a structured response containing the answer and source document information.
- **Main Success Scenario**:
  1. The user sends an HTTP POST request to the `/query` endpoint via the web server.
  2. The web server validates the request body.
  3. The web server forwards the request to the RAG system’s `/query` API.
  4. The RAG system retrieves relevant context from the vector database.
  5. The RAG system returns a structured response with the answer and source document information.
  6. The web server returns the response to the user.
- **Alternative Flows**:
  - **Invalid Query Parameter**: If the query parameter is missing or malformed, the web server returns a `400 Bad Request` error with a clear message indicating the query parameter is required.
  - **RAG Service Failure**: If the RAG service is temporarily unavailable or returns a 500 error, the web server logs the error, returns a `503 Service Unavailable` response, and maintains the request history for debugging.
  - **Invalid API Token**: If the API token is missing or invalid, the web server returns a `401 Unauthorized` response with a message indicating authentication is required and logs the failure for security auditing.
- **Business Rules and Constraints**:
  - The `/query` endpoint must accept only POST requests with a valid query parameter.
  - The web server must validate the request structure before forwarding it to the RAG system.
- **Technical Dependencies**:
  - The web server must act as a secure gateway to the RAG system.
  - The RAG system must support authentication via API token and base URL.
#### Use Case 4: Query Multiple RAG Microservices with Context (REQ202511272348-US04)
- **Primary Actor**: LightRAG User
- **Stakeholders**: Central RAG Server, Multiple RAG Microservices (e.g., retrieval, generation, indexing)
- **Preconditions**:
  - The LightRAG environment is configured with access to multiple RAG microservices.
  - The user has submitted a query with sufficient context to determine which microservices should respond.
- **Postconditions**:
  - A unified, context-aware response is returned to the user with source attribution.
- **Main Success Scenario**:
  1. The user submits a query that requires data from different microservices (e.g., user history, product catalog, real-time pricing).
  2. The LightRAG system evaluates the query context and routes it to relevant microservices.
  3. Each microservice retrieves data from its domain.
  4. The retrieved data is aggregated and validated for consistency.
  5. A unified, context-aware response is generated with clear attribution of data sources.
  6. The user receives the final response.
- **Alternative Flows**:
  - **Ambiguous Query**: If the query lacks sufficient context, the system returns a response indicating uncertainty and suggests clarifying the query or selecting specific microservices.
  - **Microservice Failure**: If one microservice is unavailable or fails, the system logs the error, continues to retrieve data from remaining functional microservices, and returns a partial result with a clear annotation about the missing data source.
  - **Malformed Query**: If the query is malformed or contains invalid syntax, the system detects the error and returns a response with correction guidance without initiating any microservice calls.
- **Business Rules and Constraints**:
  - The system must detect context gaps and respond appropriately.
  - The response must include clear source attribution for all data.
- **Technical Dependencies**:
  - The system must support dynamic routing based on query intent.
  - The microservices must have independent, well-defined interfaces.
#### Use Case 5: Query with Specified Calling Order (REQ202511272348-US05)
- **Primary Actor**: LightRAG User
- **Stakeholders**: Central RAG Server, RAG Microservices (e.g., movie database, product catalog, knowledge base)
- **Preconditions**:
  - The LightRAG server is configured with access to multiple RAG microservices.
  - The user has defined a user prompt and query question.
- **Postconditions**:
  - A context-aware response is generated and returned to the user based on the specified calling order.
- **Main Success Scenario**:
  1. The user sends a request specifying a calling order (e.g., "first retrieve from movie database, then from product catalog, then from knowledge base") and includes the user prompt and query question.
  2. The system validates the input and parses the calling order.
  3. The system validates the existence of each microservice in the calling order.
  4. The system executes the calling order sequentially, retrieving context from each microservice.
  5. The retrieved context is aggregated into a unified context pool.
  6. The final response is generated with the user prompt and returned to the user.
- **Alternative Flows**:
  - **Invalid Calling Order**: If a microservice in the calling order does not exist or is unreachable, the system returns an error message indicating the non-existent service and logs the error.
  - **Redundant Calls**: If the calling order includes a duplicate microservice (e.g., "movie database" twice), the system detects the redundancy, skips the duplicate call, and proceeds with the remaining steps.
  - **Malformed Input**: If the user prompt or query is malformed or empty, the system returns a validation error and does not initiate any microservice calls.
- **Business Rules and Constraints**:
  - The calling order must be valid and consist of existing microservices.
  - Redundant calls must be detected and skipped to avoid unnecessary processing.
- **Technical Dependencies**:
  - The system must parse and validate the calling order.
  - The microservices must support sequential execution.
#### Use Case 6: Query with Different Roles (REQ202511272348-US06)
- **Primary Actor**: LightRAG User
- **Stakeholders**: Central RAG Server, RAG Microservices (e.g., document retrieval, real-time data analysis, product recommendations)
- **Preconditions**:
  - The LightRAG platform is configured with multiple RAG microservices with distinct roles.
  - The user has submitted a query that requires information from multiple specialized microservices.
- **Postconditions**:
  - A unified, context-aware output is presented to the user.
- **Main Success Scenario**:
  1. The user sends a query that requires information from multiple specialized microservices.
  2. The system analyzes the query and identifies which microservices should handle specific aspects of the request.
  3. The system dynamically routes the query to the appropriate microservices based on their roles.
  4. Each microservice retrieves relevant data from its domain.
  5. The responses are aggregated and presented to the user as a unified, context-aware output.
- **Alternative Flows**:
  - **Ambiguous Query**: If the query is ambiguous or lacks sufficient context, the system requests clarification and returns a response indicating the ambiguity.
  - **Service Unavailability**: If a required microservice is unavailable, the system logs the failure, attempts to reroute to a fallback service, or provides a partial response based on available data.
  - **Malformed Query**: If the query is malformed, the system returns a clear error message with guidance on how to correct it.
- **Business Rules and Constraints**:
  - The routing must be based on predefined microservice roles.
  - The system must handle ambiguity and provide clear feedback to the user.
- **Technical Dependencies**:
  - The microservices must have well-defined roles and capabilities.
  - The routing engine must support role-based decision-making.
### User Tasks
#### User Task 1: Send Query with Microservice and Port (REQ202511272348-US01)
- **Interaction**: The user sends an HTTP request to the central RAG server with a query specifying a microservice name and port number.
- **Step-by-Step Flow**:
  1. The user constructs a query string with a microservice name (e.g., "Software Engineering RAG") and port number (e.g., "8080").
  2. The user sends the query via an HTTP POST request to the central RAG server.
  3. The server receives the request and validates the microservice name and port number.
  4. The server checks if the microservice exists in its configuration.
  5. If valid, the server routes the query to the microservice on the specified port.
  6. The microservice retrieves context from its vector database.
  7. The microservice returns a contextual response with source document information.
  8. The central server assembles the response and sends it to the user.
- **Input Requirements**:
  - Microservice name (string): Must match a predefined service name.
  - Port number (integer): Must be within a valid range (e.g., 8000–9999).
- **Error Handling**:
  - If the port is invalid, the server returns a clear error message with the invalid port number.
  - If the microservice is not found, the server returns a list of available microservices with their port numbers.
- **Output**:
  - A structured response containing the answer and source document information.
#### User Task 2: Perform Health Check (REQ202511272348-US02)
- **Interaction**: The user initiates a RAG workflow by checking the server health.
- **Step-by-Step Flow**:
  1. The user sends an HTTP GET request to the `/health` endpoint.
  2. The server receives the request and attempts to respond.
  3. If the server is healthy, it returns a `200 OK` response.
  4. If the server is unavailable, it returns a `503` or `404` response.
  5. If the response is malformed, the system logs a warning and triggers a fallback mechanism.
  6. If there is a network timeout, the system returns a `504 Gateway Timeout` error.
- **Input Requirements**:
  - None (only an HTTP GET request to `/health`).
- **Error Handling**:
  - A `503` or `404` response indicates the server is not available.
  - A malformed response triggers a fallback mechanism.
- **Output**:
  - A `200 OK` response indicates the server is healthy and operations can proceed.
#### User Task 3: Send Query via Web Server (`/query` API) (REQ202511272348-US03)
- **Interaction**: The user sends a query to the web server at `http://[::1]:8080`.
- **Step-by-Step Flow**:
  1. The user constructs a query string with a valid query parameter.
  2. The user sends an HTTP POST request to the `/query` endpoint.
  3. The web server validates the request body.
  4. The web server forwards the request to the RAG system’s `/query` API.
  5. The RAG system retrieves context from the vector database.
  6. The RAG system returns a structured response with the answer and source document information.
  7. The web server returns the response to the user.
- **Input Requirements**:
  - Query parameter (string): Must be present and valid.
- **Error Handling**:
  - If the query parameter is missing or malformed, the web server returns a `400 Bad Request` error.
  - If the RAG service fails, the web server returns a `503 Service Unavailable` response.
  - If the API token is invalid, the web server returns a `401 Unauthorized` response.
- **Output**:
  - A structured response with the answer and source document information.
#### User Task 4: Submit Query with Context to Multiple Microservices (REQ202511272348-US04)
- **Interaction**: The user submits a query that requires data from multiple microservices.
- **Step-by-Step Flow**:
  1. The user constructs a query with sufficient context (e.g., user history, product catalog, real-time pricing).
  2. The user sends the query to the LightRAG system.
  3. The system evaluates the query context and determines which microservices should respond.
  4. The system routes the query to relevant microservices.
  5. Each microservice retrieves its domain-specific data.
  6. The retrieved data is aggregated and validated for consistency.
  7. A unified, context-aware response is generated with source attribution.
  8. The user receives the final response.
- **Input Requirements**:
  - Query string with sufficient context to determine microservice roles.
- **Error Handling**:
  - If the query is ambiguous, the system returns a response indicating uncertainty and suggests clarification.
  - If a microservice fails, the system logs the error and returns a partial response with annotations.
- **Output**:
  - A unified, context-aware response with clear source attribution.
#### User Task 5: Send Query with Calling Order (REQ202511272348-US05)
- **Interaction**: The user sends a request specifying a calling order for microservices.
- **Step-by-Step Flow**:
  1. The user constructs a request with a user prompt, query question, and a calling order (e.g., "first from movie database, then from product catalog").
  2. The user sends the request to the LightRAG server.
  3. The system validates the input and parses the calling order.
  4. The system validates the existence of each microservice in the order.
  5. The system executes the calling order sequentially, retrieving context from each microservice.
  6. The retrieved context is aggregated into a unified context pool.
  7. The final response is generated with the user prompt and returned to the user.
- **Input Requirements**:
  - User prompt (string): Must be valid and non-empty.
  - Query question (string): Must be valid and non-empty.
  - Calling order (list of microservice names): Must contain valid, existing microservices.
- **Error Handling**:
  - If a microservice is invalid or missing, the system returns an error with diagnostic information.
  - If a duplicate microservice is specified, the system skips the duplicate and continues.
- **Output**:
  - A context-aware response generated from the aggregated microservice results.
#### User Task 6: Send Query with Role-Based Routing (REQ202511272348-US06)
- **Interaction**: The user submits a query that requires information from specialized microservices.
- **Step-by-Step Flow**:
  1. The user constructs a query that targets specific domains (e.g., document retrieval, real-time data analysis).
  2. The user sends the query to the LightRAG system.
  3. The system analyzes the query and determines which microservices should handle specific aspects.
  4. The system dynamically routes the query to the appropriate microservices based on their roles.
  5. Each microservice retrieves relevant data from its domain.
  6. The responses are aggregated and presented to the user as a unified output.
- **Input Requirements**:
  - Query string that specifies intent or domain.
- **Error Handling**:
  - If the query is ambiguous, the system requests clarification.
  - If a microservice is unavailable, the system attempts a fallback or provides a partial response.
- **Output**:
  - A unified, context-aware output with source attribution.
---
### Validation and Testing
- **Unit Testing**: Each user task must be tested with valid and invalid inputs to ensure correct error handling and response generation.
- **UAT (User Acceptance Testing)**: Real users must test the system with sample queries to verify usability and response quality.
- **Error Logging**: All error conditions must be logged with sufficient detail for debugging and monitoring.
- **Security Auditing**: All authentication failures must be logged for security compliance.
### Implementation Notes
- The central RAG server acts as a gateway, routing queries to appropriate microservices based on port and role.
- All microservices must be containerized and deployed on Kubernetes for scalability and fault tolerance.
- The system must support real-time monitoring and logging to detect and resolve issues proactively.